...
import { Storage } from '@jrcdev/boros-code-core';

describe('FileCommandLoader - Extension Commands Support', () => {
  const projectRoot = '/test/project';
  const userCommandsDir = Storage.getUserCommandsDir();
  const projectCommandsDir = path.join(projectRoot, '.boros', 'commands');
  ...
  it('should load commands from multiple extensions in alphabetical order', async () => {
    const ext1Dir = path.join(projectRoot, '.boros', 'extensions', 'ext-b');
    const ext2Dir = path.join(projectRoot, '.boros', 'extensions', 'ext-a');
